...
const OUTPUT_FILE = path.resolve(__dirname, '../entries/index.ts');
...
function getEntries(dir: string): string[] {
    ...
    list.forEach((file) => {
        ...
        if (stat && stat.isDirectory()) {
            ...
            if (file.endsWith('.ts') && !file.endsWith('.d.ts') && file !== 'types.ts' && file !== 'index.ts') {
...
function generateIndex() {
    ...
    const entries: BlogEntryFile[] = files.map((filePath, index) => {
        ...
        // Create relative import path from OUTPUT_FILE (entries/index.ts) to entry file
        // e.g., entries/2026/02/file.ts → ./2026/02/file
