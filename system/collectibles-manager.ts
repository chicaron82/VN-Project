...
// Global dependencies
declare function getNoteMetadata(noteId: string): any;
declare function isDifficultyUnlocked(requiredDifficulty: string, currentDifficulty: string): boolean;
declare class StandaloneNotesViewer {
    constructor(game: any);
    readStatus: Record<string, boolean>;
    saveReadStatus(): void;
    updateNotificationDots(): void;
    markTabAsRead(tab: string): void;
}
...
export class CollectiblesManager {
    private game: any;
    private route: any;
    private collectedNotes: Record<string, string[]>;
    private noteTimestamps: Record<string, number>;
    private allNotes: Record<string, Note>;
    private unreadCount: number;
    private readNotes: Set<string>;
    private seenNotes: Record<string, number>;
    private noteCodeDrops: Record<string, any>;
    private notesButton: HTMLElement | null;
    private notesCount: HTMLElement | null;
    private notesViewer: HTMLElement | null;
    private notesList: HTMLElement | null;
    private closeNotesButton: HTMLElement | null;

    // Navigation state
    private currentNoteId: string | null = null;
    private currentNoteList: string[] = [];

    // Event handlers
    private escKeyHandler: ((e: KeyboardEvent) => void) | null = null;
    private touchStartHandler: ((e: TouchEvent) => void) | null = null;
    private touchEndHandler: ((e: TouchEvent) => void) | null = null;
    private swipeTargetElement: HTMLElement | null = null;
    ...
    showNotesViewer() {
        ...
        // Group notes by type
        const notesByType: Record<string, (Note & { id: string })[]> = {
            z: [],
            cz: [],
            zr: [],
            gz: [],
    ...
    renderNoteSection(sectionTitle: string, notes: Note[], type: NoteCategory, allCollectedIds: string[]) {
        ...
        notes.forEach(note => {
            const isCollected = note.id && this.collectedNotes[type].includes(note.id);
...
P.S. The barback skill strikes again.`
    ...
    getSenderName(noteType: NoteCategory): string {
        const senders = {
            z: 'Z (The Architect)',
            cz: 'CZ (The Heart)',
    ...
    openNoteOverlay(noteId: string, allNoteIds: string[]) {
    ...
    setupSwipeGestures(_overlay: HTMLElement) {
    ...
    displayNoteInOverlay(noteId: string) {
        const note = this.allNotes[noteId];
    ...
    updateNavigationButtons() {
        ...
        if (nextBtn) {
            (nextBtn as HTMLButtonElement).disabled = currentIndex >= this.currentNoteList.length - 1;
            nextBtn.onclick = () => this.navigateNote(1);
        }
    }

    navigateNote(direction: number) {
...
if (typeof window !== 'undefined') {
    (window as any).CollectiblesManager = CollectiblesManager;
}
