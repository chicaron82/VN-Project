...
interface GameSettings {
    ...
    comfortMode: boolean;
    comfortIntensity: number;
}

/** Backlog entry for time machine */
interface BacklogEntry {
    character: string;
    dialogue: string;
    distorted?: boolean;  // Flag for hijacked/corrupted dialogue
    isDistorted?: boolean; // Alias
    timestamp: number;
    sceneId: string | null;
    pageIndex: number;
    isJumpable: boolean;
    routeName?: string | null;
    gameState: BacklogGameState | null;
}

/** Game state captured in backlog */
interface BacklogGameState {
...
/** GameEngine reference (permissive) */
declare class GameEngine {
    ...
    gameState: { flags: Record<string, any> };
    secretCodesManager: any;
    toggleFullscreen(): void;
    showWarningOverlay(title: string, message: string): void;
    triggerSensoryFeedback(type: string, target: any, description: string): void;
    triggerTetherReaction(changeType: string): void;
    showConfirmDialog(title: string, message: string, onConfirm: Function, showCancel?: boolean): void;
    getScenePosition(): { currentRouteId: string | null; currentSceneId: string | null; currentPageIndex: number };
    getCurrentBackgroundKey(): string | null;
    getSerializableFlags(): Record<string, any>;
    loadSceneFromSnapshot(entry: any): Promise<void>;
    applySerializableFlags(flags: any): void;
    setBackgroundByKey(bgKey: string): void;
    displayScene(scene: any, sceneId: string): void;
    closeBacklog(): void;
    uiController: any;
    [key: string]: any;
}
...
class SettingsManager {
    ...
    setupUI() {
        ...
        // Auto-Advance Toggle
        const autoToggle = document.getElementById('auto-advance-toggle') as HTMLInputElement | null;
        if (autoToggle) {
            autoToggle.checked = this.settings.autoAdvance;
            autoToggle.addEventListener('change', () => {
                this.setAutoAdvance(autoToggle.checked);
            });
        }

        // Auto-Advance Delay Slider
        const delaySlider = document.getElementById('auto-delay-slider') as HTMLInputElement | null;
        if (delaySlider) {
            delaySlider.value = String(this.settings.autoDelay);
            delaySlider.addEventListener('input', () => {
                this.setAutoDelay(parseInt(delaySlider.value));
            });
        }
        ...
        if (autoSkipPrologueToggle && autoSkipPrologueStatus) {
            ...
            if (!isUnlocked) {
                ...
                autoSkipPrologueToggle.addEventListener('change', () => {
                    this.setAutoSkipPrologue(autoSkipPrologueToggle.checked);
                });
            }
        }
        ...
        if (hapticToggle) {
            ...
            hapticToggle.addEventListener('change', () => {
                this.setHapticEnabled(hapticToggle.checked);
            });
        }

        // DIZEE: Comfort Mode Toggle
        const comfortModeToggle = document.getElementById('comfort-mode-toggle') as HTMLInputElement | null;
        if (comfortModeToggle) {
            // Set initial state from settings
            comfortModeToggle.checked = this.settings.comfortMode;

            // Listen for changes
            comfortModeToggle.addEventListener('change', () => {
                this.setComfortMode(comfortModeToggle.checked);
            });
        }

        // TORI'S ADDITION: Comfort Intensity Slider 💚
        const intensitySlider = document.getElementById('comfort-intensity-slider') as HTMLInputElement | null;
        if (intensitySlider) {
            // Set initial value from settings
            intensitySlider.value = String(this.settings.comfortIntensity ?? 1);

            // Listen for changes - triggers live preview
            intensitySlider.addEventListener('input', () => {
                const level = Number(intensitySlider.value);
        ...
        const tutorialToggle = document.getElementById('tutorial-hints-toggle') as HTMLInputElement | null;
        ...
        const resetTutorialsBtn = document.getElementById('reset-tutorials-btn') as HTMLButtonElement | null;
        ...
        document.querySelectorAll<HTMLButtonElement>('.tether-difficulty-btn').forEach(btn => {
            btn.addEventListener('click', () => {
    ...
    setupTabSystem() {
        // Tab switching
        document.querySelectorAll<HTMLButtonElement>('.settings-tab-btn').forEach(btn => {
            btn.addEventListener('click', () => {
    ...
    updateUI() {
        ...
        const intensitySliderEl = document.getElementById('comfort-intensity-slider') as HTMLInputElement | null;
        ...
        if (intensitySliderEl) {
            ...
            intensitySliderEl.value = String(savedIntensity);
        ...
        const autoSkipToggleEl = document.getElementById('auto-skip-prologue-toggle') as HTMLInputElement | null;
        const autoSkipStatus = document.getElementById('auto-skip-prologue-status');
        if (autoSkipToggleEl && autoSkipStatus) {
            const isUnlocked = this.game.skipPrologueUnlocked;

            if (isUnlocked) {
                autoSkipToggleEl.disabled = false;
                autoSkipToggleEl.checked = this.settings.autoSkipPrologue;
                autoSkipStatus.textContent = this.settings.autoSkipPrologue ? 'ON' : 'OFF';
                autoSkipStatus.style.color = ''; // Reset to default
            } else {
                autoSkipToggleEl.disabled = true;
                autoSkipToggleEl.checked = false;
    ...
    commitToInsaneMode() {
        ...
        // @ts-ignore - getDifficultyProfile is external global
        const insaneProfile = typeof getDifficultyProfile !== 'undefined' ? getDifficultyProfile('insane') : { holdOn: { hidden: true } };
    ...
    setupSecretCodesUI() {
        ...
        const codeInput = document.getElementById('secret-code-input') as HTMLInputElement | null;
...
class BacklogManager {
    ...
    addEntry(character: string, dialogue: string, isDistorted: boolean = false) {
        ...
        let sceneIdString: string | null = null;
        if (this.game.currentScene) {
            if (typeof this.game.currentScene === 'string') {
                ...
                // @ts-ignore - gameState.progress is dynamic
...
═══════════════════════════════════════`;
    ...
    jumpToScene(sceneId: string | any, _pageIndex: number) {
        ...
        } else if ((route as any).act1 && (route as any).act1[sceneIdString]) {
            sceneFunction = (route as any).act1[sceneIdString];
            context = (route as any).act1;
            // @ts-ignore - Route has dynamic act properties
        } else if ((route as any).act2 && (route as any).act2[sceneIdString]) {
            sceneFunction = (route as any).act2[sceneIdString];
            context = (route as any).act2;
            // @ts-ignore - Route has dynamic act properties
        } else if ((route as any).act3 && (route as any).act3[sceneIdString]) {
            sceneFunction = (route as any).act3[sceneIdString];
            context = (route as any).act3;
            // @ts-ignore - Route has dynamic endings property
        } else if ((route as any).endings && (route as any).endings[sceneIdString]) {
            sceneFunction = (route as any).endings[sceneIdString];
            context = (route as any).endings;
        }
...
if (typeof window !== 'undefined') {
    ...
    (window as any).BacklogManager = BacklogManager;
}
