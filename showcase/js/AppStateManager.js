...
class AppStateManager {
    constructor() {
        this.STORAGE_KEY = 'uv7-app-states';
        this.VERSION = 1;
        this.MAX_APPS = 10; // LRU limit

        // Debounce settings
        this.scrollDebounceMs = 300;
    ...
    init() {
        ...
        window.addEventListener('uv7:state:changed', (e) => {
     ...
     * @param {Object} detail - { appId, state, preview }
     */
     ...
     */
    autoGeneratePreview(appId, state, theme) {
        const preview = {
            type: 'gradient',
            gradient: [theme.primary, theme.secondary],
            effect: theme.effect,
     ...
     */
    createScrollHandler(appId, tabId) {
        return (scrollPosition) => {
            ...
            this.scrollDebounceTimers[key] = setTimeout(() => {
        ...
        switch (appId) {
            case 'showcase':
                ...
                if (typeof TIMELINE_DATA !== 'undefined' && TIMELINE_DATA.entries) {
                    const latestEntry = TIMELINE_DATA.entries[0];
                    if (latestEntry && latestEntry.sortDate) {
                        const latestDate = new Date(latestEntry.sortDate).getTime();
                        if (latestDate > lastVisited) {
                            return TIMELINE_DATA.entries.filter(e =>
                                new Date(e.sortDate).getTime() > lastVisited
                            ).length;
                        }
                    }
                }
...
// Global instance
window.UV7AppStateManager = new AppStateManager();
