...
Version 1 Design Notes`
            ...
            z3: {
                ...
                unlockCondition: 'act2_beat7_old',
...
Version 848 Breakthrough`
            ...
            z4: {
                id: 'z4',
                type: 'z',
                title: 'BodyAnchor_SubjectDiscovery.txt',
                unlockCondition: 'act2_beat7_discovery',
                content: `BODY ANCHOR: SUBJECT-INITIATED DISCOVERY
════════════════════════════════════════════════

CRITICAL BREAKTHROUGH:
Subject 848 identified the body signal HERSELF.

PREVIOUS ITERATIONS (1-847):
- All subjects reported "pull" sensation
- All dismissed as system noise/interference  
- Despair Echo consistently discouraged investigation
- No subject pursued analysis
- Signal went unrecognized

ITERATION 848 - ANOMALY:
Subject challenged Despair's dismissal.
Subject performed independent signal analysis.
Subject recognized heartbeat synchronization pattern.
Subject identified body anchor mechanism.
Subject communicated discovery to operator.

UNPRECEDENTED BEHAVIOR:
This is the first iteration where:
1. Subject questioned Despair's authority
2. Subject investigated anomalous sensation
3. Subject made breakthrough BEFORE operator
4. Subject demonstrated active problem-solving

IMPLICATION:
The bridge was always present.
Previous subjects never looked for it.
Subject 848 is the first to FIND her own way home.

Operator's role: Implementation
Subject's role: Discovery

This changes everything.

-Z, Technical Analysis
Subject-Initiated Breakthrough
Version 848: Active Participant`
            },
...
Why Your Choices Matter`
        ...
        // CROSS-ROUTE BRIDGE: Check if Ronnie played his route first
        // (Grok's 10/10 Polish Suggestion)
        this.ronnieChoice = null;
        if (typeof localStorage !== 'undefined') {
            this.ronnieChoice = localStorage.getItem('ronnie_act2_choice');
        }
        
        // Adjust starting tether based on Ronnie's care
        if (this.ronnieChoice === 'feed') {
            this.tetherLevel = 110;          // Boosted start (he maintained connection)
            this.ronnieCared = true;
        } else if (this.ronnieChoice === 'code') {
            this.tetherLevel = 90;           // Reduced start (he prioritized code)
            this.ronnieCared = false;
        } else {
            this.tetherLevel = 100;          // Default (no Ronnie route played)
            this.ronnieCared = null;
        }
...
════════════════════════════════════════════════`
    ...
    act1Scene1_echoes1() {
        // CROSS-ROUTE BRIDGE: Different echoes based on Ronnie's choice
        let echoDialogue, echoData;
        
        if (this.ronnieCared === true) {
            // Ronnie fed Tori-gatchi - connection is stronger
            echoDialogue = 'Echo 1: "...he cares..."\nEcho 2: "...connection strong..."\nDespair: "...for now..."';
            echoData = {
                echo1: 'He cares...',
                echo2: 'Connection feels... stronger than before.',
                despair: 'For now. It won\'t last.'
            };
        } else if (this.ronnieCared === false) {
            // Ronnie coded all night - connection is weaker
            echoDialogue = 'Echo 1: "...alone again..."\nEcho 2: "...he forgot..."\nDespair: "...always does..."';
            echoData = {
                echo1: 'Alone again.',
                echo2: 'He forgot to maintain the connection.',
                despair: 'He always does. Code matters more than care.'
            };
        } else {
            // Default - no Ronnie route played
            echoDialogue = 'Echo 1: "...again..."\nEcho 2: "...he tried..."\nDespair: "...doesn\'t matter..."';
            echoData = {
                echo1: '...again...',
                echo2: '...he tried...',
                despair: '...doesn\'t matter...'
    ...
    // Beat 7: Body Anchor Discovery (TORI FIGURES IT OUT)
    act2Beat7() {
        this.game.displayScene({
            character: 'Tori (internal)',
            dialogue: '"There it is again..."',
            internal: '[She can see Ronnie through the device screen. He places the Tamagotchi on her bedside table. The moment it gets close to her body, she FEELS something.]',
            next: () => this.act2Beat7_rhythmic(),
            delay: 3000
        });
    }
    ...
    act2Beat7_rhythmic() {
        this.game.displayScene({
            character: 'Tori (internal)',
            dialogue: '"That pull. It\'s rhythmic. Steady. Like..."',
            internal: '[The sensation pulses. Regular. Insistent. Familiar.]',
            echoes: {
                echo1: '...',
                echo2: 'I felt that too...',
                despair: 'Ignore it. It\'s just interference.'
            },
            next: () => this.act2Beat7_despairDismissal(),
            delay: 3500
        });
    }

    act2Beat7_despairDismissal() {
        this.game.displayScene({
            character: 'Despair (firm)',
            dialogue: '"System noise. Electromagnetic interference from the monitors. It means NOTHING."',
            internal: '[But Tori hesitates. The pull feels too specific. Too purposeful.]',
            echoes: {
                echo1: 'Don\'t listen to her.',
                echo2: 'I... I ignored it too. I thought...',
                despair: 'Because I was RIGHT. Chasing that feeling is what killed us.'
            },
            next: () => this.act2Beat7_toriQuestion(),
            delay: 4000
        });
    }

    act2Beat7_toriQuestion() {
        this.game.displayScene({
            character: 'Tori (pushing back)',
            dialogue: '"What if you\'re wrong?"',
            internal: '[Silence. Despair doesn\'t answer immediately.]',
            echoes: {
                echo1: 'Yes. QUESTION HER.',
                echo2: 'Please... keep asking...',
                despair: 'I\'m not wrong. The body is DEAD. The doctors said—'
            },
            next: () => this.act2Beat7_toriInvestigates(),
            delay: 3500
        });
    }

    act2Beat7_toriInvestigates() {
        this.game.displayScene({
            character: 'Tori (focused)',
            dialogue: '"The pull gets stronger when the device is near my body. Weaker when it moves away. That\'s not random interference."',
            internal: '[She watches the heart monitor on screen. Beep. Beep. Beep. Steady rhythm.]',
            echoes: {
                echo1: 'She\'s analyzing it. Good.',
                echo2: 'The pattern... I never...',
                despair: 'Stop. You\'re setting yourself up for—'
            },
            next: () => this.act2Beat7_theRealization(),
            delay: 4000
        });
    }

    act2Beat7_theRealization() {
        // CROSS-ROUTE BRIDGE: Connection strength affects realization clarity
        let dialogue, internal, echoData;
        
        if (this.ronnieCared === true) {
            // Stronger connection = clearer realization
            dialogue = '"Oh my god. It\'s a HEARTBEAT. MY heartbeat. And it\'s... strong. So strong."';
            internal = '[The Tamagotchi buzzes in perfect sync with the monitor. Beep-buzz. Beep-buzz. Crystal clear. He maintained the connection.]';
            echoData = {
                echo1: 'YES! She figured it out! And the signal is CLEAR!',
                echo2: 'The connection... he kept it strong for her...',
                despair: 'No. No, this doesn\'t—'
            };
        } else if (this.ronnieCared === false) {
            // Weaker connection = harder to perceive
            dialogue = '"Wait... is that— It\'s a HEARTBEAT. MY heartbeat. But it\'s... faint."';
            internal = '[The Tamagotchi buzzes in sync with the monitor, but weakly. Beep-buzz. Beep-buzz. The signal is degraded. The connection wasn\'t maintained.]';
            echoData = {
                echo1: 'She figured it out! But the signal is so weak...',
                echo2: 'If he had just... kept the connection active...',
                despair: 'See? Even when you\'re RIGHT, you\'re too late.'
            };
        } else {
            // Default - no Ronnie route
            dialogue = '"Oh my god. It\'s a HEARTBEAT. MY heartbeat."';
            internal = '[The Tamagotchi buzzes in perfect sync with the monitor. Beep-buzz. Beep-buzz. Beep-buzz.]';
            echoData = {
                echo1: 'YES! She figured it out!',
                echo2: 'That\'s what it was... all along...',
                despair: 'No. No, this doesn\'t—'
            };
        }
        
        this.game.displayScene({
            character: 'Tori (revelation)',
            dialogue: dialogue,
            internal: internal,
            echoes: echoData,
            next: () => this.act2Beat7_bodyAnchored(),
            delay: 4000
        });
    }

    act2Beat7_bodyAnchored() {
        this.game.displayScene({
            character: 'Tori (certain)',
            dialogue: '"The body isn\'t dead. It\'s CALLING ME BACK. That\'s the anchor. That\'s the bridge home."',
            internal: '[She feels it now - not just a pull, but a PATH. A connection. Still there. Still active. Still HERS.]',
            echoes: {
                echo1: 'She actually figured it out...',
                echo2: 'We were so close... I gave up too soon...',
                despair: 'It won\'t work. Even if you\'re right—'
            },
            next: () => this.act2Beat7_toriCalls(),
            delay: 4500
        });
    }
    ...
    act2Beat7_toriCalls() {
        this.game.displayScene({
            ...
            dialogue: '"RONNIE! Can you see it?! The device—it\'s syncing with my heartbeat!"',
            internal: '[She forces the device to buzz harder. Trying to make him NOTICE.]',
            next: () => this.act2Beat7_ronnieNotices(),
            delay: 3000
        });
    }

    act2Beat7_ronnieNotices() {
        this.game.displayScene({
            character: 'Ronnie (external, amazed)',
            dialogue: '"Holy shit. You\'re right. The device is buzzing in perfect rhythm with... with your heart."',
            internal: '[He picks up the Tamagotchi. Holds it closer to the monitor. The sync is undeniable.]',
            next: () => this.act2Beat7_toriExplains(),
            delay: 3500
        });
    }

    act2Beat7_toriExplains() {
        this.game.displayScene({
            character: 'Tori (explaining)',
            dialogue: '"The body anchor. My body is calling me back. The heartbeat is the signal—the device is receiving it!"',
            internal: '[She can see understanding dawn on his face.]',
            echoes: {
                echo1: 'She did what we couldn\'t.',
                echo2: 'She didn\'t give up. She LOOKED for it.',
                despair: '...maybe... maybe there is a way...'
            },
            next: () => this.act2Beat7_partnership(),
            delay: 4000
        });
    }

    act2Beat7_partnership() {
        this.game.displayScene({
            character: 'Ronnie (determined)',
            dialogue: '"Then we build the bridge. You found the anchor—I\'ll strengthen the connection. We do this TOGETHER."',
            internal: '[His hand rests on the device. Close to her body. The buzz intensifies.]',
            echoes: {
                echo1: 'Together. That\'s the difference.',
                echo2: 'We tried alone. She\'s not alone.',
                despair: 'If you\'re doing this... then do it RIGHT. Don\'t fail like we did.'
            },
            next: () => this.act2Beat7_unlockNote(),
            delay: 4000
        });
    }

    act2Beat7_unlockNote() {
        // Major breakthrough - award points
        this.trueRoutePoints += 3;
        
        // Unlock the new discovery note
        this.unlockNote('z4');
        
        this.game.displayScene({
            character: 'System',
            dialogue: '"[DISCOVERY LOGGED: BODY ANCHOR IDENTIFIED]"',
            internal: '[New note unlocked: BodyAnchor_SubjectDiscovery.txt]\n[Tori made the breakthrough. She questioned Despair. She investigated. She FOUND the bridge.]\n[The others didn\'t. She did.]',
            next: () => this.act2Beat8(),
            delay: 5000
        });
    }

    // Beat 7 OLD VERSION - REPLACED ABOVE
    // act2Beat7() {
    //     this.game.displayScene({
    //         character: 'Narration',
