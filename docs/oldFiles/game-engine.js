...
class GameEngine {
    constructor() {
        ...
        // Character Sprite Elements (Version 848.2)
    ...
    displayScene(sceneData) {
        ...
        // Handle character sprites (if specified in sceneData)
        if (sceneData.sprites) {
            // Left sprite
            if (sceneData.sprites.left) {
                if (sceneData.sprites.left === 'none') {
                this.hideSprite('left');
            } else {
        ...
        if (sceneData.sprites.right) {
            if (sceneData.sprites.right === 'none') {
                this.hideSprite('right');
            } else {
    ...
    // Handle active speaker highlighting (dim non-speaker)
    if (sceneData.character) {
        const speakerName = sceneData.character.toLowerCase();
    
        // Check which sprite is speaking based on character name
        if (speakerName.includes('ronnie')) {
            this.setActiveSpeaker('left'); // Ronnie on left
        } else if (speakerName.includes('tori')) {
            this.setActiveSpeaker('right'); // Tori on right
        } else if (speakerName === 'narration' || speakerName === 'system') {
            this.setActiveSpeaker(null); // No dimming for narration
        }
    }
